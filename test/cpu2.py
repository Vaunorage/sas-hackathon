...
NBCPT = 4 # max number of accounts to take
NB_SC = 10 # max number of external scenarios
NB_AN_PROJECTION = 10 # max number of external projection years
NB_SC_INT = 10 # max number of internal scenarios
NB_AN_PROJECTION_INT = 10 # max number of internal projection years
CHOC_CAPITAL = 0.35
HURDLE_RT = 0.10
...
def faithful_calculs_macro(population: pd.DataFrame, hash_tables):
    ...
    for ext_year in range(1, NB_AN_PROJECTION+1):
        ext_populations = []
        for ext_scn in range(1, NB_SC+1):
            ext_populations = population.copy()
            res = population.apply(lambda x:faithful_cash_flow_calculation(population_row=x, hash_tables=hash_tables,
                                                 current_year=ext_year, scenario_type="EXTERNE"), axis=1)
            ext_populations.append(res)
